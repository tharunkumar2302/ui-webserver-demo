...
const paginate = (schema) => {
   ...
   */
  schema.statics.paginate = async function (filter, options = false) {

    console.log(filter,options,'pagination');
    let sort = '';
    if(!options.sortBy){
    ...
    const limit = options && options.limit && parseInt(options.limit, 10) > 0 ? parseInt(options.limit, 10) : 100;
    ...
    return Promise.all([countPromise, docsPromise]).then((values) => {
      ...
      console.log(err, "pagination Data");
    });
  };
};
